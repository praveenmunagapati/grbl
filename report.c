...
void report_grbl_settings() {
  ...
  report_util_float_setting(30,0.0,N_DECIMAL_RPMVALUE);
  report_util_float_setting(31,0.0,N_DECIMAL_RPMVALUE);
...
void report_ngc_parameters()
{
  ...
  report_util_feedback_line_feed();
  // Tool length offset and probe are not supported in Pen Plotter Edition
}
...
void report_realtime_status()
{
  ...
  if (bit_isfalse(settings.status_report_mask,BITFLAG_RT_STATUS_POSITION_TYPE) ||
    ...
    for (idx=0; idx< N_AXIS; idx++) {
      // Apply work coordinate offsets to current position.
