...
// type of command that is called in the block.
...
#define MODAL_GROUP_G13 9 // [G61] Control mode
...
// compile smaller. Necessary due to being completely out of flash on the 328p.
...
typedef struct {
  ...
  float coord_system[N_AXIS];    // Current work coordinate system (G54+).
  float coord_offset[N_AXIS];    // Retains the G92 coordinate offset relative to machine zero in mm.
} parser_state_t;
